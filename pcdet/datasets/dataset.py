...
class DatasetTemplate(torch_data.Dataset):
    ...
    def partition(self, points, num=10, max_dis=60, rate=0.2):
        ...
        for i in range(num):
            ...
            if i == num - 1:
                ...
                # (previous + this bin + future bins)/ all < rate
    ...
    def input_point_discard_bin_density(self, points, bin_num=3, rate=0.8):
        """
        discard points by a bin-based sampling.
        """
        retain_rate = 1 - rate
        ...
        pts_per_bin_num = int((points_num - distant_points_num_acc) / bin_num + 0.0001)

        for i in range(len(parts) - pos, len(parts)):
            # density regulated retain rate
            density_bin = parts[i].shape[0] / pts_per_bin_num
            density_bin = np.sqrt(density_bin)
            regulated_rate = retain_rate / (density_bin + 0.0001)
            regulated_rate = min(regulated_rate, 1)
            regulated_rate = max(regulated_rate, 0.1)
            output_pts_num = int(points.shape[0] * regulated_rate)
            retain_this_bin_num = int((output_pts_num - distant_points_num_acc) / (pos + 0.0001))
            # retain_pts = int(regulated_rate * pts_per_bin_num)
            retain_pts = min(retain_this_bin_num, parts[i].shape[0])

            # retain random pts_per_bin points
            # if parts[i].shape[0] > pts_per_bin_num:
            rands = np.random.permutation(parts[i].shape[0])
            parts[i] = parts[i][rands[:retain_pts]]
        ...
        if data_dict.get('gt_boxes', None) is not None:
            ...
            for i in range(self.rot_num):
                ...
                if 'gt_boxes' + rot_num_id in data_dict:
                    ...
                    gt_classes = np.array([self.class_names.index(n) + 1 for n in data_dict['gt_names']],
                                          dtype=np.int32)
                    gt_boxes = np.concatenate(
                        (data_dict['gt_boxes' + rot_num_id], gt_classes.reshape(-1, 1).astype(np.float32)), axis=1)
            ...
            if 'mm' in data_dict:
                if self.dataset_cfg.get('LATER_FUSION', True):
                    ...
                    if self.training:
                        ...
                        # points_mm2 = self.input_point_discard_bin_density(points_mm, rate=self.input_discard_rate)
                    else:
                        points_mm2 = self.input_point_discard(points_mm, bin_num=10, rate=self.input_discard_rate)
                        # points_mm2 = self.input_point_discard_bin_density(points_mm, bin_num=10,
                        #                                                   rate=self.input_discard_rate)
                    ...
                    if self.training:
                        ...
                        points_mm2 = self.input_point_discard_bin_density(points_mm, bin_num=10, rate=self.input_discard_rate)
                        # points_mm2 = self.input_point_discard_bin_density(points_mm, bin_num=10,
                        #                                                   rate=self.input_discard_rate)
