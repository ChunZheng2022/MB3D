...
class VirConv8x(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size,  **kwargs):
        ...
        num_filters_mm = model_cfg.NUM_FILTERS_MM
        ...
        if self.model_cfg.get('MM', False):
            self.vir_conv1 = NRConvBlock(input_channels, num_filters_mm[0], stride=1, indice_key='vir1')
            self.vir_conv2 = NRConvBlock(num_filters_mm[0], num_filters_mm[1], stride=2, indice_key='vir2')
            self.vir_conv3 = NRConvBlock(num_filters_mm[1], num_filters_mm[2], stride=2, indice_key='vir3')
            self.vir_conv4 = NRConvBlock(num_filters_mm[2], num_filters_mm[3], stride=2, padding=(0, 1, 1),
                                          indice_key='vir4')
            self.vir_conv5 = NRConvBlock(num_filters_mm[3], num_filters_mm[4], stride=2, indice_key='vir5')
            self.vir_conv6 = NRConvBlock(num_filters_mm[4], num_filters_mm[5], stride=2, padding=(0, 1, 1),
                                          indice_key='vir6')
        ...
        if self.return_num_features_as_dict:
            ...
            num_point_features.update({
                ...
                'x_conv5': num_filters_mm[4],
                'x_conv6': num_filters_mm[5],
            })
    ...
    def forward(self, batch_dict):
        ...
        for i in range(rot_num):
            ...
            if self.model_cfg.get('MM', False):
                ...
                newx_conv5 = self.vir_conv5(newx_conv4, batch_size, calib, 16, self.x_trans_train, trans_param)

                newx_conv6 = self.vir_conv6(newx_conv5, batch_size, calib, 16, self.x_trans_train, trans_param)
