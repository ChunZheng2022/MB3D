...
class CFCMHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, point_cloud_range=None, voxel_size=None,  num_class=1,
        ...
        self.stage_cat = self.rot_num * self.rot_num
    ...
    def roi_grid_pool_mm(self, batch_dict, i):
        ...
        for k, src_name in enumerate(self.pool_cfg_mm.FEATURES_SOURCE):
            ...
            # if src_name in ['x_conv1', 'x_conv2', 'x_conv3', 'x_conv4']:
            if src_name in ['x_conv1', 'x_conv2', 'x_conv3', 'x_conv4', 'x_conv5', 'x_conv6']:
    ...
    def probability_based_routing(self,stage_num):
        probabilities = self.p_list
        assert len(probabilities) == n, "The length of the probability distribution list should be stage_num"
        assert np.isclose(sum(probabilities), 1, rtol=1e-3), "The sum of probability distributions should be 1"
    ...
    def multi_grid_pool_aggregation(self, batch_dict, targets_dict):
        ...
        for i in range(self.rot_num):
            ...
            if self.training:
                r_stage = self.probability_based_routing(self.rot_num)
                targets_dict = self.assign_targets(batch_dict, r_stage)
            ...
            final_feat = torch.cat([cur_feat_mm, cur_feat],-1) # NPC
